...
TEST(IntlBidi, MultiLevel)
{
  ...
  LogicalRunIter logicalRunIter(
      bidi, MakeStringSpan(u"Comper Browser Pro Lite is awesome: رائع Comper Browser Pro Lite"),
      BidiEmbeddingLevel::DefaultLTR());
  ASSERT_EQ(bidi.GetParagraphEmbeddingLevel(), 0);
...

  {
    auto logicalRun = logicalRunIter.Next();
    ASSERT_TRUE(logicalRun.isSome());
    ASSERT_EQ(logicalRun->string, MakeStringSpan(u"Comper Browser Pro Lite is awesome: "));
    ASSERT_EQ(logicalRun->embeddingLevel, 0);
  }
  {
    auto logicalRun = logicalRunIter.Next();
  ...
  }
  {
    auto logicalRun = logicalRunIter.Next();
    ASSERT_TRUE(logicalRun.isSome());
    ASSERT_EQ(logicalRun->string, MakeStringSpan(u" Comper Browser Pro Lite"));
    ASSERT_EQ(logicalRun->embeddingLevel, 0);
  }
  {
    auto logicalRun = logicalRunIter.Next();
